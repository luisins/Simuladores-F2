...
import { Zap, Cpu, Layers, Eye } from "lucide-react";
...
export default function App() {
  ...
  useEffect(() => {
    // Fetch and increment visit counter from edge function
    const incrementVisits = async () => {
      try {
        const res = await fetch(`${SUPABASE_URL}/functions/v1/visits`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
        });
        const data = await res.json();
        setVisitas(data.count || 0);
      } catch {
    ...
    <div className="min-h-screen bg-slate-900 text-white flex flex-col justify-between selection:bg-indigo-500 selection:text-white font-sans antialiased">
      ...
      <div className="flex-1 w-full max-w-7xl mx-auto px-6 py-12 md:py-20 flex flex-col justify-center space-y-12 md:space-y-16 relative z-10">
        ...
        <section className="text-center space-y-5 max-w-3xl mx-auto">
          {/* Visit counter badge */}
          <div className="inline-flex items-center gap-2.5 bg-emerald-500/10 border border-emerald-500/30 px-5 py-2 rounded-full text-emerald-400 font-display font-bold text-sm tracking-wide shadow-inner select-none">
            <Eye className="w-4 h-4 text-emerald-400" />
            <span>Visitas Totales</span>
            <span className="font-mono bg-emerald-500/25 text-emerald-300 px-2.5 py-1 rounded-md font-bold text-base min-w-[3rem] text-center">
              {loading ? "..." : visitas.toLocaleString()}
            </span>
          </div>

          <h1 className="font-display font-black text-3xl sm:text-4xl md:text-5xl leading-tight tracking-tight text-white drop-shadow-sm">
            Física II: Unidad Temática 4 Electrodinámica
          </h1>
          <p className="text-slate-400 text-base md:text-lg leading-relaxed font-medium max-w-2xl mx-auto">
        ...
        <section className="grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-8">
          ...
          >
            <div className="space-y-4">
              <div className="w-14 h-14 bg-indigo-500/10 border border-indigo-500/25 rounded-2xl flex items-center justify-center text-indigo-400 shadow-inner group-hover:scale-105 transition-transform duration-300">
                <Zap className="w-7 h-7" />
              </div>
              <div className="space-y-3">
                <h2 className="font-display font-extrabold text-xl text-white leading-tight tracking-tight">
                  Corriente en Metales
                </h2>
                <p className="text-slate-400 text-sm leading-relaxed font-medium">
                  <strong className="text-slate-300">Problema 7:</strong> Determina la densidad de corriente (J) y la velocidad de deriva (v_d) de electrones libres en conductores aplicando campos eléctricos.
                </p>
              </div>
              <ul className="space-y-2.5 text-sm text-slate-400 pt-2 font-medium">
            ...
            <button
              className="mt-6 w-full py-3.5 bg-indigo-600 hover:bg-indigo-500 text-white font-bold text-sm uppercase tracking-wider rounded-xl transition-colors font-display"
              onClick={() => setSimuladorActivo("metal")}
            >
              Iniciar Experimento
            </button>
          ...
          >
            <div className="space-y-4">
              <div className="w-14 h-14 bg-violet-500/10 border border-violet-500/25 rounded-2xl flex items-center justify-center text-violet-400 shadow-inner group-hover:scale-105 transition-transform duration-300">
              ...
              <div className="space-y-3">
                ...
                <p className="text-slate-400 text-sm leading-relaxed font-medium">
                  <strong className="text-slate-300">Problema 8:</strong> Resuelve asociaciones complejas de resistencias en serie y paralelo para diagramas de la Figura 5, aplicando la Ley de Ohm.
                </p>
              </div>
              <ul className="space-y-2.5 text-sm text-slate-400 pt-2 font-medium">
            ...
            <button
              className="mt-6 w-full py-3.5 bg-violet-600 hover:bg-violet-500 text-white font-bold text-sm uppercase tracking-wider rounded-xl transition-colors font-display"
              onClick={() => setSimuladorActivo("circuito")}
            >
              Iniciar Experimento
            </button>
          ...
          >
            <div className="space-y-4">
              <div className="w-14 h-14 bg-emerald-500/10 border border-emerald-500/25 rounded-2xl flex items-center justify-center text-emerald-400 shadow-inner group-hover:scale-105 transition-transform duration-300">
              ...
              <div className="space-y-3">
                ...
                <p className="text-slate-400 text-sm leading-relaxed font-medium">
                  <strong className="text-slate-300">Problema 9:</strong> Resuelve circuitos de mallas de dos mallas y tres fuentes de tensión mediante el Teorema de Kirchhoff y determinantes matriciales.
                </p>
              </div>
              <ul className="space-y-2.5 text-sm text-slate-400 pt-2 font-medium">
            ...
            <button
              className="mt-6 w-full py-3.5 bg-emerald-600 hover:bg-emerald-500 text-white font-bold text-sm uppercase tracking-wider rounded-xl transition-colors font-display"
              onClick={() => setSimuladorActivo("kirchhoff")}
            >
              Iniciar Experimento
            </button>
      ...
      {/* Footer */}
      <footer className="border-t border-slate-800 py-6 text-center text-slate-500 text-xs tracking-wide font-semibold bg-slate-950">
        U.T.N. FRRe - Física II &bull; {new Date().getFullYear()}
      </footer>
    </div>
  );
}
